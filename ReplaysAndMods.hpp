...
namespace ReplaysAndMods {
	...
	ReplayDetails parseReplayHeader(Range&& replay) {
		...
		static constexpr auto modInfoSize = std::size_t{ 22 };
	...
	std::vector<char> fixReplay(Range&& replay) {
		...
		try {
			...
			while (true) {
				...
				static constexpr auto zeroes = std::string_view{ "\0\0\0\0"sv };
			...
			static constexpr auto finalData = std::string_view{ "\x02\x1A\x00\x00\x00"sv };
	...
	std::optional<std::vector<std::wstring>> getModSkudefPathsFromReplay(const ReplayDetails& replay) {
		using namespace Internal;
		static constexpr auto ra3 = std::wstring_view{ L"ra3" };
